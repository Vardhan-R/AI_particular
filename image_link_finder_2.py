...
def main(rating_int: int):
    rating_str = chr(-5 * rating_int ** 2 + 3 * rating_int + 115) # 0 ==> s; 1 ==> q; 2 ==> e
...
old_images_target = int(sys.argv[1]) # [file_name, old_images_target]

PATH = "D:\Program Files (x86)\chromedriver.exe"
driver = webdriver.Chrome(PATH)

for rating_int in range(3):
    main(rating_int)

driver.quit()

# end of program update
fp = open(f"all_categories_communication.txt", 'w')
fp.write("1")
fp.close()
